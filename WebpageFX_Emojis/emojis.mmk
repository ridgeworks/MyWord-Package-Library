@doc
	...
	This simple *emojis* package uses the [WebpageFX emoji] cheat sheet to map emoji names to their corresponding
	graphics. The type `emoji` maps a text name to an `<img>` element referencing the corresponding `.png` file on the
	cheat sheet. A default style of `{width:3ex; height:3ex; vertical-align: bottom;}` is applied; use CSS selector
	`img.my_emoji` to change it.
